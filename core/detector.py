...
Detection dataclass now includes lat/lon GPS fields populated by main.py
...
class Detection:
    ...
    cx: int; cy: int
    # World coordinates (metres, East/North from calibration origin)
    wx: Optional[float] = field(default=None)
    wy: Optional[float] = field(default=None)
    # GPS coordinates (populated in main.py after homography transform)
    ...
    def to_dict(self) -> dict:
        return {
            ...
            # lat/lon added conditionally in main.py to keep CSV clean
            # when gps_enabled=False they are omitted from the dict there
        }
