#!/usr/bin/env python
"""
interfaces/traffic_ui_tracked.py
========================
Custom PyQt5 UI with three tabs:
  Tab 1 — ROI & Setup        : load video frame, draw polygon ROI, draw boxes to track
...
Run standalone:
    python interfaces/traffic_ui_tracked.py
"""
...
class ROICanvas(QLabel):
    ...
    def __init__(self, parent=None):
        ...
        self._static_boxes: list[dict] = [] # [{"x1": x1, "y1": y1, "x2": x2, "y2": y2, "class_name": class_name, "frame_idx": frame_idx}]
    ...
    def _redraw(self) -> None:
        ...
        if self._static_boxes:
            ...
            for idx, box in enumerate(self._static_boxes):
                ...
                label = f"T{idx+1}: {box['class_name']} [F{box.get('frame_idx', 0)}]"
                painter.setFont(QFont("Arial", 8, QFont.Bold))

                label_rect = QRectF(p1.x(), p1.y() - 15, 110, 15)
                painter.drawRect(label_rect)
                painter.setPen(QPen(Qt.black))
                painter.drawText(label_rect, Qt.AlignCenter, label)
                painter.setPen(pen_box)

        # Draw current drag preview (Boxes drawing mode)
        ...
        if self._mode == "roi" and not self._closed:
            ...
            painter.drawText(
                QRectF(8, 8, 500, 20), Qt.AlignLeft,
                "Mode: Draw Tracked Boxes  |  Click & Drag to draw rectangular bounding boxes"
            )
    ...
    def run(self):
        try:
            ...
            if self.static_boxes:
                self.log.emit(f"[Trackers] Active — {len(self.static_boxes)} manual tracked boxes registered")
            ...
            while True:
                ...
                # Update or initialize trackers for manually drawn boxes
                for mt in manual_trackers:
                    if not mt["active"]:
                        continue
...
class ROITab(QWidget):
    ...
    def _build_ui(self):
        ...
        self.btn_mode_boxes = QPushButton("⬜ Draw Tracked Boxes")
        ...
        # Tracked Boxes Section
        self.lbl_boxes_section = QLabel("Tracked Box Controls")
        ...
        self.lbl_boxes_count = QLabel("Tracked Boxes: 0")
    ...
    def _on_box_drawn(self, x1, y1, x2, y2):
        ...
        box_data = {"x1": x1, "y1": y1, "x2": x2, "y2": y2, "class_name": cls, "frame_idx": frame_idx}
    ...
    def _update_boxes_list(self):
        ...
        for idx, box in enumerate(self.canvas._static_boxes):
            self.list_boxes.addItem(
                f"T{idx+1}: {box['class_name']} @ F{box.get('frame_idx', 0)} ({box['x1']},{box['y1']}) to ({box['x2']},{box['y2']})"
            )
        self.lbl_boxes_count.setText(f"Tracked Boxes: {len(self.canvas._static_boxes)}")
    ...
    def _save_roi(self):
        ...
        QMessageBox.information(
            self, "Setup Saved",
            f"Saved ROI with {len(pts)} points and {len(static_boxes)} tracked boxes.\n"
...
class AnalysisTab(QWidget):
    ...
    def set_roi(self, pts: list, static_boxes: list, video_path: str = ""):
        ...
        self.lbl_roi_status.setText(
            f"✅ Setup: {n} ROI pts, {b} tracked boxes"
        )
        if video_path:
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Aerial Traffic Analytics (Manual Tracking Support)")
        ...
        # Wire signals (Passing ROI points and manual boxes to Analysis)
        ...
        self.statusBar().showMessage(
            "Ready  |  Tab 1: Setup ROI & Manual Boxes to Track → Tab 2: Run Analysis → Tab 3: View Trajectories")
